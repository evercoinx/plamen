...
ADAPTIVE_DEPTH_LOOP(findings_inventory):
  ...
  // ═══ CARGO-FUZZ CAMPAIGN (Soroban - conditional) ═══
  ...
  //   Log: "CARGO_FUZZ_AVAILABLE=false → proptest fallback in Phase 5"
...
1. **Boundary Substitution**: Substitute boundary values into expressions. Tag: `[BOUNDARY:X=val → outcome]`
   Soroban examples: `[BOUNDARY:i128::MAX → overflow if overflow-checks=false]`, `[BOUNDARY:ttl=0 → entry archived before next read]`, `[BOUNDARY:allowance=0 → transfer_from reverts or no-op]`
2. **Parameter Variation**: Vary inputs across valid range. Tag: `[VARIATION:param A→B → outcome]`
   Soroban examples: `[VARIATION:storage Instance→Persistent → eviction window differs]`, `[VARIATION:token SAC→custom → transfer_from hook fired]`, `[VARIATION:invoke_contract→try_invoke_contract → panic vs Result]`
3. **Trace to Termination**: Follow execution to terminal state. When a boundary value produces weight=0, contribution=0, or amount=0, trace whether the zero-value entry still increments a counter or passes a gate that downstream code relies on for correctness. **Nested call resolution**: When tracing through a cross-contract call, also trace what happens when control returns to the caller — does it reload storage and verify post-call state? Tag: `[TRACE:path→outcome at L{N}]`

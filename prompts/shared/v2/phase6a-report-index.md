...
5. **Proven-only demotion** (ONLY if the resolved configuration says `PROVEN_ONLY: true`): For each finding whose BEST evidence tag is `[CODE-TRACE]` (no `[POC-PASS]`, `[MEDUSA-PASS]`, `[PROD-ONCHAIN]`, `[PROD-SOURCE]`, or `[PROD-FORK]`), cap severity at Low. Record the original severity in the "Trust Adj." column as `PROVEN(original_sev)`. Count total demotions for the report header note: *"Proven-only mode enabled: {N} findings capped at Low from {severities} due to unproven evidence ([CODE-TRACE] only)."* If `PROVEN_ONLY: false`, this rule is disabled and `[CODE-TRACE]` does not change severity.
   ...
   **EXCEPTION (structural-untestability carve-out)**: do NOT cap when the verify file's PoC ledger reason is a GENUINE structural-untestability blocker that is VALID for the finding's PoC Class — `STRUCTURAL_NO_EXECUTABLE_HARM_ASSERTION` for a structural/integration class finding, `EXTERNAL_DEPENDENCY_NO_FORK_OR_ADDRESS` (where the verifier's own prose does not say the dependency was mockable), or `DEPLOYMENT_ONLY_REQUIRES_LIVE_EXTERNAL` with no local harness. In that case PRESERVE the verifier-stated severity and record `STRUCTURAL-UNTESTABLE(sev)` in the Trust Adj. column (NOT `PROVEN(sev)`). A `[CODE-TRACE]` with NO valid ledger blocker is STILL capped at Low with `PROVEN(sev)`: a test was not written though a harness exists; `NO_BUILD_ENVIRONMENT` cited while `build_status.md` reports SUCCESS; `PURE_SPEC_OR_DOCS_ONLY`; or `STRUCTURAL_NO_EXECUTABLE_HARM_ASSERTION` on a unit/property class finding. The driver enforces this distinction mechanically in `severity_binding.md`; comply with the binding value.

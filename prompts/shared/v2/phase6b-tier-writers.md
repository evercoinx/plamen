...
7. **Per-section minimum length**: Every `### [X-NN]` section body (after the header) MUST contain at least 400 characters. Sections shorter than this are considered stubs and will fail the post-report quality gate, triggering a retry of the tier writer with an identical prompt.

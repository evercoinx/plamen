...
def _ai_title_stub(path, session_id="abde1e79"):
    ...
    _write_jsonl(
        path,
        {"type": "ai-title", "aiTitle": "Execute bake", "sessionId": session_id},
    )
...
def test_wait_completes_on_idle_footer_when_transcript_is_aititle_stub(tmp_path):
    ...
    state = session.wait_for_turn_complete(
        timeout_s=5.0, quiescence_s=0.05, poll_s=0.01, transcript_poll_s=0.01
    )

    assert state.complete is True
    assert state.done_seen is True
    assert time.time() - start < 4.0  # returned early, did NOT hit timeout
...
def test_wait_does_not_complete_on_midturn_output_with_stub(tmp_path):
    ...
    session.is_alive = lambda: True  # type: ignore[method-assign]
    with session._recent_output_lock:
        session._recent_output = "Analyzing files... esc to interrupt"

    state = session.wait_for_turn_complete(
        timeout_s=0.2, quiescence_s=0.05, poll_s=0.01, transcript_poll_s=0.01
    )
